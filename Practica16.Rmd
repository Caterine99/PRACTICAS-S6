---
title: "UNIDAD 3: Práctica 16 - Simulación del Teorema del Límite Central"
...
Teorema del Límite Central (TLC) informa acerca de la distribución de muestreo de medias de muestras con tamaño n . Recuérdese que básicamente existen tres tipos de información que se desea conocer sobre una distribución:
...
El Teorema del Límite Central establece que sí las observaciones $X_{1},X_{2},X_{3},...,X_{n}$ son variables aleatorias independientes e idénticamente distribuidas con una distribución de probabilidad cualquiera y en la cual cada una de ellas tenga la misma media $\mu$ y la misma varianza $\sigma^{2}$ (ambas finitas).

Entonces el promedio muestral tiene una distribución con media $\mu$ y varianza $\frac{\sigma^{2}}{n}$ que tiende hacia $N(0, 1)$ a medida que n tiende a $\infty$.
...
```{r}
getwd()
setwd("C:/Users/user/OneDrive/Paquete R/PRACTICAS-S6")
```
...
-   Ejemplo 1: Generar 100 números aleatorios de una distribución binomial con parámetros n=10 (número de ensayos o pruebas), y p=0.25 (probabilidad de éxito)
...
```{r}
hist(Z, main="Histograma de Z ~ N(0, 1)", xlab="z = número binomiales estándarizados", ylab="f(z)", prob=TRUE, col="khaki")
...
El teorema central del límite establece que $\frac{ \bar{X}-\mu}{\sigma\sqrt{n}}$ \~ N(0,1)

-   **ejemplo 2:**Suponga que $X_{1}$ es normal con media $\mu= 5$ y desviación estándar $\alpha = 5$. Entonces necesitamos una función para encontrar el valor de $\frac{ \bar{X}-\mu}{\sigma\sqrt{n}}$
...
**6°) Simular el Teorema del Límite Central con datos exponencial Un ejemplo de una distribución sesgada es la exponencial.**

Necesitamos conocer que sí tiene media $\mu=10$ , entonces la desviación estándar $\sigma$ es también 10, por eso sólo necesitamos especificar la media.
...
simulExp(mu, m, n)
hist(MediasEstand, main="Medias Exp(10); n=1", xlab="m medias exp estándarizadas", prob=TRUE, col="darkolivegreen3")
...
n<-5
simulExp(mu, m, n)
hist(MediasEstand, main="Medias Exp(10); n=5", xlab="m medias exp estándarizadas", prob=TRUE, col="darkolivegreen3")
...
n<-15
simulExp(mu, m, n)
hist(MediasEstand, main="Medias Exp(10); n=15", xlab="m medias exp estándarizadas", prob=TRUE, col="darkolivegreen3")
...
n<-50
simulExp(mu, m, n)
hist(MediasEstand, main="Medias Exp(10); n=50", xlab="m medias exp estándarizadas", prob=TRUE, col="darkolivegreen3")
